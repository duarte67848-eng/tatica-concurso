...
export default function Dificeis({ colors }: { colors?: any }) {
  ...
  async function loadDificeis(email: string) {
    ...
    if (diffs && diffs.length > 0) {
      const ids = diffs.map(f => f.questao_id);
    ...
    return (
      <div>
        ...
        <div style={{ background: c.backgroundSecondary, padding: "2rem", borderRadius: "8px", textAlign: "center" }}>
          ...
          <p style={{ color: c.textSecondary, marginTop: "0.5rem" }}>Durante os exercícios, clique no ícone 🎯 para marcar uma questão como difícil.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        <h1 style={{ color: c.gold }}>🎯 Questões Difíceis ({questoes.length})</h1>
          ...
          {questoes.map((q, i) => (
            <div key={q.id} style={{ background: c.backgroundSecondary, border: `1px solid ${c.border}`, borderRadius: "8px", padding: "1rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.5rem" }}>
                ...
                <button onClick={() => { const userData = JSON.parse(window.sessionStorage.getItem("tatica_user") || "{}"); supabase.from("questoes_dificeis").delete().eq("usuario_email", userData.email).eq("questao_id", parseInt(q.id)); setQuestoes(questoes.filter(x => x.id !== q.id)); }} style={{ background: c.red, color: "#fff", border: "none", padding: "4px 8px", borderRadius: "4px", cursor: "pointer", fontSize: "0.75rem" }}>Remover</button>
              </div>
